...
export class ZiroomPlatformAccessory {
  ...
  constructor(readonly platform: ZiroomHomebridgePlatform, readonly accessory: PlatformAccessory<Device>) {
    platform
      .request<{ name: '品牌' | '型号' | 'mac'; message: string }[]>(API_URL.getDevBaseInfo, {
      ...
      .then((res) => {
        ...
        this.services.forEach((service) => {
          service.setCharacteristic(this.platform.Characteristic.Name, accessory.context.device.devName);
        });
  ...
  generateService<T extends typeof Service>(service: T[]) {
    this.services = service.map((item) => this.accessory.getService(item as any) || this.accessory.addService(item as any));
  }

  async getDevice(type: Ziroom.Actions) {
    const { device } = this.accessory.context;
    if (!this.deviceDetailPromise) {
      this.deviceDetailPromise = this.platform.request<Ziroom.Device>(API_URL.getDeviceDetail, {
    ...
    try {
      ...
      this.platform.log.debug(`Get ${device.devName} ->`, JSON.stringify(deviceDetail));
  ...
  async setDevice(devElement: Ziroom.Device['groupInfoMap'][string]['devElementList'][number], param?: string) {
    const { device } = this.accessory.context;
    try {
      ...
      this.platform.log.debug(`Set ${device.devName} ->`, devElement.operName, JSON.stringify(result));
      return result;
    } catch (error) {
